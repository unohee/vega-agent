...
async def system_check():
    """선택 의존성 상태 — 설치 마법사·설정 창 표시용.

    코드 실행(bash_exec/python_exec/office/chart)은 호스트 동봉 인터프리터로 직접
    동작하므로 Docker 같은 외부 의존성이 필요 없다 (INT-1870 Phase C). 안전은
    path_guard + 런타임 가드 + permission mode 가 담당. 진단용 정보만 반환."""
    import platform
    return JSONResponse({
        "platform": platform.system(),
        "code_exec": {
            "mode": "host",      # 호스트 동봉 인터프리터 — Docker 불필요
            "available": True,
            "hint": None,
        },
    })
