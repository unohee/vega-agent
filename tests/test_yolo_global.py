...
# Dependencies: web.state (상태), web.server (slash handler)
# Test Status: 검증 중
...
def test_global_off_by_default():
    assert _state.yolo_on("any-sid") is False


def test_global_on_applies_to_all_sessions():
    """전역을 켜면 어떤 sid에서도 YOLO가 켜진 것으로 본다 — sid 불일치 버그 방지."""
    _state._YOLO_GLOBAL = True
    assert _state.yolo_on("session-A") is True
    assert _state.yolo_on("session-B") is True
    assert _state.yolo_on("never-seen-sid") is True


def test_session_local_still_works():
    """하위호환 — 세션별 _YOLO_MODE도 여전히 인정."""
    _state._YOLO_MODE["only-this"] = True
    assert _state.yolo_on("only-this") is True
    assert _state.yolo_on("other") is False
...
def test_global_or_local():
    ...
    _state._YOLO_GLOBAL = True
    _state._YOLO_MODE.clear()
    assert _state.yolo_on("x") is True
    _state._YOLO_GLOBAL = False
    _state._YOLO_MODE["x"] = True
    assert _state.yolo_on("x") is True
...
def test_slash_yolo_sets_global():
    ...
    assert _state._YOLO_GLOBAL is True
    assert _state.yolo_on("sid-2") is True


def test_slash_yolo_off_clears_global_and_local():
    """/yolo-off가 전역 + 세션별 잔재를 모두 끈다."""
    _state._YOLO_GLOBAL = True
    _state._YOLO_MODE["leftover"] = True
    server.handle_slash("/yolo-off", "sid-1")
    assert _state._YOLO_GLOBAL is False
    assert _state.yolo_on("leftover") is False
    assert len(_state._YOLO_MODE) == 0
