...
# Dependencies: web.state (상태), web.server (비즈니스 로직)
# Test Status: 검증 중
...
def _isolate(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(_state, "_yolo_flag_path", lambda: tmp_path / "yolo_global.flag")
    monkeypatch.setattr(_state, "_autopilot_path", lambda: tmp_path / "autopilot.json")
    _state._YOLO_GLOBAL = False
    _state._YOLO_MODE.clear()
    yield
    _state._YOLO_GLOBAL = False
    _state._YOLO_MODE.clear()
...
def test_yolo_global_persists_to_disk():
    _state.save_yolo_global(True)
    assert _state._load_yolo_global() is True
    _state.save_yolo_global(False)
    assert _state._load_yolo_global() is False
...
def test_autopilot_register_unregister():
    sid = "auto-1"
    _state.autopilot_register(sid)
    assert sid in _state._load_autopilot()
    _state.autopilot_unregister(sid)
    assert sid not in _state._load_autopilot()


def test_autopilot_register_idempotent():
    sid = "auto-2"
    _state.autopilot_register(sid)
    _state.autopilot_register(sid)  # 두 번 등록해도 resumes 리셋 안 됨
    data = _state._load_autopilot()
...
async def test_db_resume_stops_at_cap(monkeypatch):
    ...
    _state.autopilot_register(sid)
    data = _state._load_autopilot()
    data[sid]["resumes"] = _state.HEARTBEAT_DB_MAX_RESUMES
    _state._save_autopilot(data)
    called = {"n": 0}
    def _boom(s):
        called["n"] += 1
...
async def test_db_resume_revives(monkeypatch):
    ...
    assert called["n"] == 1
    assert _state._load_autopilot()[sid]["resumes"] == 1
